...
public class UserPage {
	...
	public List<User_TR_Board_DTO> print_total_trBoard(String user_id) {
		List<User_TR_Board_DTO> mypageDTO_list = new ArrayList<>();
		try {
			mypageDTO_list= utb_DAO.getTotalList_by_userId(user_id);
		...
		return mypageDTO_list;
	}
	
	// 해당 유저의 ThingRank 카테고리별 출력 (순위 재조정함)
	public List<User_TR_Board_DTO> print_category_trBoard(String user_id,String category_name) {
		int new_rank = 1; // 카테고리별 출력 시 순위 다시 나타내기 위한 변수
		int temp;
		List<User_TR_Board_DTO> mypageDTO_list= new ArrayList<>();
		try {
			mypageDTO_list= utb_DAO.getCateList_by_userId(user_id,category_name);
			if(mypageDTO_list.isEmpty()) {
				System.out.println("No results were found for your search");
			}else {
				for (User_TR_Board_DTO myPageDTO : mypageDTO_list) {

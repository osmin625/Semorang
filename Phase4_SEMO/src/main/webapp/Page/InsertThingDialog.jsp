    ...
    <div id="temp">
       	<form method= "POST"  name = "insert_form" >
            ...
            <span>
                ...
                <!-- 
                 <button type = "button" id = vc value ="valid_check" name ="thing_valid_check" 
                		onClick = "location.href= '${pageContext.request.contextPath }/ThingValidCheck.main'">valid_check</button>
                 -->
				<c:choose>
            ...
            <span>
            	<c:choose>
					<c:when test = "${insert_result == 0 }">${thing_name}을 추가하지 못했습니다.</c:when>
					<c:when test = "${insert_result == 1 }">${thing_name}이 추가되었습니다. </c:when>
				</c:choose>
            </span>
        </form>
    </div>
